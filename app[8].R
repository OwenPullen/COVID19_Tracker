...
df = Germany
## Graphs
GraphWales <- ggplot(data = Wales, 
  aes(x = date, y = people_vaccinated)) + 
   geom_line(color = "ForestGreen", size = 2) +
    labs(x = "Date", y = "No. People Given 1st Vaccine Dose (Wales)", 
     main = "Number of People Recieved 1st Vaccine Dose in Wales")
plot(GraphWales)
### Interactive plot
### EP: you're so close to getting this to work!
### country_vac_plot was a static plot but if you turn it into a function (as below) then shiny can render it 
### using the relevant input df
...
# example of variable in global environment that can be picked up in app
text = "COVID-19 Vaccine Tracker"


### UI -------------------------------------------------------------------------------------
ui <- navbarPage(theme = shinytheme("sandstone"), title = "Owen's 1st Vaccine Tracker",
                 tabPanel("Writing",
                          mainPanel(
                            textOutput("country_text"))),
             
             tabPanel("Panel 2",
                      plotOutput("panel_2_plot")),
             ...
             ### EP: I have commented out the lines below to ensure the input 'country' is only defined once
             
             # tabPanel("Menu",
             #          selectInput("country", "Country",
             #                      c("Germany", "France", "Brazil"),
             #                      selected = "Germany"),
                      ...
                      sidebarLayout(
                        sidebarPanel(
                          pickerInput("country", "Select Country",
                           choices = c("Germany", "France", "Brazil", "Wales", "UK", "Angola", "India", "Italy", "Israel", "Denmark"),
                           selected = c("Germany"),
                           multiple = FALSE)),
                          mainPanel(
                            tabsetPanel(
            ...
            tabPanel("Fully Vaccinated Plots",
                     sidebarLayout(
                       sidebarPanel(
                         pickerInput("fullvac", "Select Country",
                          choices = c("Germany", "France", "Brazil", "Wales", "UK", "South Korea", "India", "Italy", "Israel", "Denmark"),
                          selected = c("Germany"),
                           multiple = FALSE)),
                      mainPanel(
                        tabsetPanel(
...
server <- function(input, output, session) {
  ...
  output$country_text <- renderText({"1502641, 31707594 and 18859274 people given 1st
                                    vaccine doses in Wales, UK, Brazil, respectively."})
  
  output$panel_2_plot <- renderPlot({ 
  ...
  output$hist <- renderPlot({
    hist(rnorm(input$num), main = "rnorm Histogram of Chosen Number", xlab = "rnorm of chosen number", col = 'forest green')
  })
